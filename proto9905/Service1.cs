    ...
    public class UpdateRegistry
    {
      ...
      public bool LoadRegistrySettings()
      {
        try
        {
          ...
          mVersionMajor = (int)(GetRegistryValue(key, "Major Version", 0));
          mVersionMinor = (int)(GetRegistryValue(key, "Minor Version", 0));
          mVersionBuild = (int)(GetRegistryValue(key, "Build Version", 0));
          mIsValid = true;
          return true;
        }
...
mTimer.Interval = 1000 * 15; // every 15 seconds
    ...
    {
      System.Diagnostics.EventLog.WriteEntry("PROTOv7", message);
    }
